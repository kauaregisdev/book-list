...
@router.put('/books/{user_id}', response_model=schemas.Book)
async def update_book(book: schemas.BookUpdate, user_id: int, db: AsyncSession = Depends(database.get_db), current_user: schemas.User = Depends(auth.get_current_user)):
    db_book = await db.get(models.Book, user_id)
    if not db_book:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Book not found")
    if db_book.owner != current_user:
...
@router.delete('/books/{user_id}', status_code=status.HTTP_204_NO_CONTENT)
async def delete_book(user_id: int, db: AsyncSession = Depends(database.get_db), current_user: schemas.User = Depends(auth.get_current_user)):
    db_book = await db.get(models.Book, user_id)
    if not db_book:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Book not found")
    if db_book.owner != current_user:
    ...
    return HTTPException(status_code=status.HTTP_204_NO_CONTENT, detail="Book deleted successfully")